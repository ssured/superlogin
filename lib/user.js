...
module.exports = function (config, userDB, couchAuthDB, mailer, emitter) {
      ...
      .then(function() {
        ...
        if(typeof user.personalDBs === 'object') {
          ...
          Object.keys(user.personalDBs).forEach(function(finalDBName) {
            var finalURL = config.getItem('dbServer.protocol') + newSession.token + ':' + newSession.password + '@' + config.getItem('dbServer.host') + '/' + finalDBName;
            userDBs[user.personalDBs[finalDBName].name] = finalURL;
          });
  ...
  function sendMail(mailOptions) {
    ...
    if(config.getItem('testMode.noEmail')) {
      ...
      transport = nodemailer.createTransport('SMTP', config.getItem('mailer.settings'));
    }
    var sendEmail = BPromise.promisify(transport.sendMail, transport);
